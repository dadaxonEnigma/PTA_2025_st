...
translations = {
    "uz": {
        ...
        "clear_chat_btn": "🗑️ Suhbatni tozalash",
...
**Maslahat:** Eng yaxshi natija uchun barg yaxshi yoritilgan va kadrning katta qismini egallashi kerak.
        ...
        "weather_risk_header": "🌤️ Joriy ob-havo va kasallik xavfi",
        "weather_location": "📍 Joylashuv",
        "weather_temp": "🌡️ Harorat",
        "weather_humidity": "💧 Namlik",
        "weather_risk_high": "⚠️ Joriy namlik ({humidity}%) va harorat ({temp}°C) — bu «{disease}» kasalligining tarqalishi uchun qulay sharoit. Profilaktika choralarini kuchaytirish va o‘simlikni muntazam tekshirish tavsiya etiladi.",
        "weather_risk_low": "✅ Joriy sharoitlar (namlik {humidity}%, harorat {temp}°C) «{disease}» kasalligining faol rivojlanishi uchun unchalik qulay emas. Shunga qaramay, o‘simlikni muntazam kuzatib turing.",
        "weather_unavailable": "Ob-havo ma'lumotlari mavjud emas",
        "forecast_header": "📅 Ob-havo prognozi va model tavsiyalari",
        "forecast_select_date": "Sanani tanlang",
        "forecast_risk_high": "📅 {date} kuni taxminan {humidity}% namlik va {temp}°C harorat kutilmoqda. Bizning model yuklangan rasm asosida o‘simlikda **«{disease}»** kasalligini aniqladi (ishonchlilik — {confidence}%). Ana shu tashxisni hisobga olganda, ko‘rsatilgan sanadagi sharoitlar kasallikning faollashishi va tarqalishi xavfini oshiradi — shu sababli profilaktika tadbirlarini oldindan rejalashtirish tavsiya etiladi.",
        "forecast_risk_low": "📅 {date} kuni taxminan {humidity}% namlik va {temp}°C harorat kutilmoqda. Bizning model yuklangan rasm asosida o‘simlikda **«{disease}»** kasalligini aniqladi (ishonchlilik — {confidence}%). Shu tashxisga ko‘ra, ko‘rsatilgan kundagi sharoitlar kasallikning faol rivojlanishi uchun unchalik xavfli emas — ammo o‘simlikni kuzatishni davom ettirishni unutmang.",
        "get_weather_btn": "Ob-havoni olish",
        "weather_error": "Ob-havo ma'lumotlarini olishda xatolik yuz berdi",
        "temperature": "🌡️ Harorat",
        "humidity": "💧 Namlik",
        "disease_risk": "Kasallik xavfi",
        "high_risk": "Yuqori",
        "low_risk": "Past",
        "visualization_tabs": [
            "Ehtimollik grafigi",
...
**Tip:** For best results, ensure the leaf is well-lit and occupies most of the frame.
        ...
        "weather_risk_header": "🌤️ Current weather & disease risk",
        "weather_location": "📍 Location",
        "weather_temp": "🌡️ Temperature",
        "weather_humidity": "💧 Humidity",
        "weather_risk_high": "⚠️ Current humidity ({humidity}%) and temperature ({temp}°C) create favorable conditions for «{disease}» to spread. Consider stepping up preventive measures and inspecting the plant regularly.",
        "weather_risk_low": "✅ Current conditions (humidity {humidity}%, temperature {temp}°C) are less favorable for «{disease}» to develop actively. Still, keep inspecting the plant regularly.",
        "weather_unavailable": "Weather data unavailable",
        "forecast_header": "📅 Weather forecast & model-based recommendations",
        "forecast_select_date": "Select a date",
        "forecast_risk_high": "📅 On {date}, expect around {humidity}% humidity and {temp}°C. Based on the uploaded photo, our model diagnosed the plant with **«{disease}»** (confidence — {confidence}%). Given that diagnosis, the forecast conditions on this date raise the risk of the disease becoming more active and spreading — it's worth planning preventive treatment ahead of time.",
        "forecast_risk_low": "📅 On {date}, expect around {humidity}% humidity and {temp}°C. Based on the uploaded photo, our model diagnosed the plant with **«{disease}»** (confidence — {confidence}%). Given that diagnosis, the forecast conditions on this date are not particularly favorable for the disease to develop actively — but keep monitoring the plant regardless.",
...
**Совет:** Для лучших результатов лист должен быть хорошо освещён и занимать большую часть кадра.
        ...
        "weather_risk_header": "🌤️ Текущая погода и риск болезни",
        "weather_location": "📍 Местоположение",
        "weather_temp": "🌡️ Температура",
        "weather_humidity": "💧 Влажность",
        "weather_risk_high": "⚠️ Текущая влажность ({humidity}%) и температура ({temp}°C) создают благоприятные условия для распространения болезни «{disease}». Рекомендуется усилить профилактические меры и регулярно осматривать растение.",
        "weather_risk_low": "✅ Текущие условия (влажность {humidity}%, температура {temp}°C) менее благоприятны для активного развития болезни «{disease}». Тем не менее продолжайте регулярно осматривать растение.",
        "weather_unavailable": "Данные о погоде недоступны",
        "forecast_header": "📅 Прогноз погоды и рекомендации модели",
        "forecast_select_date": "Выберите дату",
        "forecast_risk_high": "📅 На {date} ожидается влажность около {humidity}% и температура {temp}°C. По загруженному фото наша модель диагностировала у растения **«{disease}»** (точность — {confidence}%). С учётом этого диагноза прогнозируемые условия в этот день повышают риск активизации и распространения болезни — стоит заранее запланировать профилактическую обработку.",
        "forecast_risk_low": "📅 На {date} ожидается влажность около {humidity}% и температура {temp}°C. По загруженному фото наша модель диагностировала у растения **«{disease}»** (точность — {confidence}%). С учётом этого диагноза прогнозируемые условия в этот день не особо благоприятны для активного развития болезни — но всё равно продолжайте наблюдать за растением.",
        "get_weather_btn": "Узнать погоду",
        "weather_error": "Не удалось получить данные о погоде",
        "temperature": "🌡️ Температура",
        "humidity": "💧 Влажность",
        "disease_risk": "Риск болезни",
        "high_risk": "Высокий",
        "low_risk": "Низкий",
        "visualization_tabs": [
            "График вероятностей",
